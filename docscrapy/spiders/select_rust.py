...
class RustSpider(scrapy.Spider):
    ...
    start_urls = ['https://docs.rs/select/latest/select/all.html']
    root_url = 'https://docs.rs'
    std_url = 'https://docs.rs/select/latest/select/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('select/css'):
            os.makedirs('select/css')
        html = response.text
        ...
        f = open('select/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('select/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('select/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
