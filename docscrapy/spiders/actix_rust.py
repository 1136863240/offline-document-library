...
class RustSpider(scrapy.Spider):
    ...
    start_urls = ['https://docs.rs/actix/latest/actix/all.html']
    root_url = 'https://docs.rs'
    std_url = 'https://docs.rs/actix/latest/actix/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('actix_rust/css'):
            os.makedirs('actix_rust/css')
        html = response.text
        ...
        html_list4 = response.xpath('//ul[@class="attributes docblock"]/li/a/@href').extract()
        for item in html_list4:
            yield scrapy.Request('%s%s' % (self.std_url, item), self.sub_parse)
            html = html.replace(item, item.split('/')[-1])
        ...
        f = open('actix_rust/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('actix_rust/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('actix_rust/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
