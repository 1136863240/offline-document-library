...
class RustSpider(scrapy.Spider):
    name = 'ffmpeg_next_rust'
    allowed_domains = ['docs.rs']
    start_urls = ['https://docs.rs/ffmpeg-next/latest/ffmpeg_next/all.html']
    root_url = 'https://docs.rs'
    std_url = 'https://docs.rs/ffmpeg-next/latest/ffmpeg_next/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('ffmpeg_next_rust/css'):
            os.makedirs('ffmpeg_next_rust/css')
        html = response.text
        ...
        f = open('ffmpeg_next_rust/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('ffmpeg_next_rust/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('ffmpeg_next_rust/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
