...
class RustSpider(scrapy.Spider):
    ...
    start_urls = ['https://docs.rs/rust-lzma/latest/lzma/all.html']
    root_url = 'https://docs.rs'
    std_url = 'https://docs.rs/rust-lzma/latest/lzma/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('rust-lzma/css'):
            os.makedirs('rust-lzma/css')
        html = response.text
        ...
        f = open('rust-lzma/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('rust-lzma/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('rust-lzma/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
