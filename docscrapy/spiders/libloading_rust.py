...
class RustSpider(scrapy.Spider):
    ...
    start_urls = ['https://docs.rs/libloading/latest/libloading/all.html']
    root_url = 'https://docs.rs'
    std_url = 'https://docs.rs/libloading/latest/libloading/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('libloading/css'):
            os.makedirs('libloading/css')
        html = response.text
        ...
        f = open('libloading/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('libloading/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('libloading/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
