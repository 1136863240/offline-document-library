...
class RustSpider(scrapy.Spider):
    ...
    start_urls = ['https://docs.rs/avl_tree/latest/avl_tree/tree/index.html']
    root_url = 'https://docs.rs'
    std_url = 'https://docs.rs/avl_tree/latest/avl_tree/tree/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('avl_tree/css'):
            os.makedirs('avl_tree/css')
        html = response.text
        ...
        html_list = response.xpath('//tr[@class=" module-item"]/td/a/@href').extract()
        ...
        f = open('avl_tree/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('avl_tree/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('avl_tree/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
