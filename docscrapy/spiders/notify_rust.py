...
class RustSpider(scrapy.Spider):
    ...
    start_urls = ['https://docs.rs/notify/6.1.1/notify/all.html']
    root_url = 'https://docs.rs'
    std_url = 'https://docs.rs/notify/6.1.1/notify/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('notify_rust/css'):
            os.makedirs('notify_rust/css')
        html = response.text
        ...
        f = open('notify_rust/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('notify_rust/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('notify_rust/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
