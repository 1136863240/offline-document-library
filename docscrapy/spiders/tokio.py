...
class RustSpider(scrapy.Spider):
    ...
    start_urls = ['https://docs.rs/tokio/latest/tokio/all.html']
    root_url = 'https://docs.rs'
    std_url = 'https://docs.rs/tokio/latest/tokio/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('tokio/css'):
            os.makedirs('tokio/css')
        html = response.text
        ...
        f = open('tokio/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('tokio/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('tokio/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
