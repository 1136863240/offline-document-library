...
class RustSpider(scrapy.Spider):
    ...
    start_urls = ['https://docs.rs/ffmpeg/latest/ffmpeg/all.html']
    root_url = 'https://docs.rs'
    std_url = 'https://docs.rs/ffmpeg/latest/ffmpeg/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('ffmpeg_rust/css'):
            os.makedirs('ffmpeg_rust/css')
        html = response.text
        ...
        html_list2 = response.xpath('//ul[@class="macros docblock"]/li/a/@href').extract()
        for item in html_list2:
            yield scrapy.Request('%s%s' % (self.std_url, item), self.sub_parse)
            html = html.replace(item, item.split('/')[-1])
        ...
        html_list7 = response.xpath('//ul[@class="constants docblock"]/li/a/@href').extract()
        ...
        f = open('ffmpeg_rust/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('ffmpeg_rust/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('ffmpeg_rust/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
