...
class RustSpider(scrapy.Spider):
    ...
    start_urls = ['https://docs.rs/rocket_contrib/latest/rocket_contrib/all.html']
    root_url = 'https://docs.rs'
    std_url = 'https://docs.rs/rocket_contrib/latest/rocket_contrib/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('rocket_contrib/css'):
            os.makedirs('rocket_contrib/css')
        html = response.text
        ...
        f = open('rocket_contrib/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('rocket_contrib/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('rocket_contrib/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
