...
class WinrsSpider(scrapy.Spider):
    name = 'winrs'
    allowed_domains = ['microsoft.github.io']
    start_urls = ['https://microsoft.github.io/windows-docs-rs/doc/windows/all.html']
    root_url = 'https://microsoft.github.io/windows-docs-rs/doc/'
    std_url = 'https://microsoft.github.io/windows-docs-rs/doc/windows/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('winrs/css'):
            os.makedirs('winrs/css')
        html = response.text
        ...
        for item in html_list:
            ...
            spli = item.split('/')
            html = html.replace(item, spli[len(spli) - 1])
        ...
        f = open('winrs/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('winrs/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('winrs/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
