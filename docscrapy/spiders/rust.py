...
class RustSpider(scrapy.Spider):
    ...
    start_urls = ['https://doc.rust-lang.org/std/all.html']
    root_url = 'https://doc.rust-lang.org/'
    std_url = 'https://doc.rust-lang.org/std/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('rust/css'):
            os.makedirs('rust/css')
        html = response.text
        ...
        f = open('rust/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('rust/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('rust/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
