...
class RustSpider(scrapy.Spider):
    ...
    start_urls = ['https://docs.rs/flate2/latest/flate2/all.html']
    root_url = 'https://docs.rs'
    std_url = 'https://docs.rs/flate2/latest/flate2/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('flate2_rust/css'):
            os.makedirs('flate2_rust/css')
        html = response.text
        ...
        f = open('flate2_rust/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('flate2_rust/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('flate2_rust/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
