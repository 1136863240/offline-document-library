...
class RustSpider(scrapy.Spider):
    ...
    start_urls = ['https://docs.rs/serde_json/latest/serde_json/all.html']
    root_url = 'https://docs.rs'
    std_url = 'https://docs.rs/serde_json/latest/serde_json/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('serde_json/css'):
            os.makedirs('serde_json/css')
        html = response.text
        ...
        f = open('serde_json/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('serde_json/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('serde_json/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
