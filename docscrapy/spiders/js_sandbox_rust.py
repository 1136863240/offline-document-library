...
class RustSpider(scrapy.Spider):
    name = 'js_sandbox_rust'
    allowed_domains = ['docs.rs']
    start_urls = ['https://docs.rs/js-sandbox/latest/js_sandbox/all.html']
    root_url = 'https://docs.rs'
    std_url = 'https://docs.rs/js-sandbox/latest/js_sandbox/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('js_sandbox_rust/css'):
            os.makedirs('js_sandbox_rust/css')
        html = response.text
        ...
        html_list = response.xpath('//section[@id="main"]/ul/li/a/@href').extract()
        ...
        f = open('js_sandbox_rust/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('js_sandbox_rust/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('js_sandbox_rust/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
