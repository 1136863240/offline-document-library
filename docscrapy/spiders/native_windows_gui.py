...
class RustSpider(scrapy.Spider):
    name = 'native_windows_gui'
    allowed_domains = ['docs.rs']
    start_urls = ['https://docs.rs/native-windows-gui/latest/native_windows_gui/all.html']
    root_url = 'https://docs.rs'
    std_url = 'https://docs.rs/native-windows-gui/latest/native_windows_gui/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('native_windows_gui/css'):
            os.makedirs('native_windows_gui/css')
        html = response.text
        ...
        html_list = response.xpath('//section[@id="main-content"]/ul/li/a/@href').extract()
        ...
        f = open('native_windows_gui/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('native_windows_gui/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('native_windows_gui/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
