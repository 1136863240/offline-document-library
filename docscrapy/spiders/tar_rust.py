...
class RustSpider(scrapy.Spider):
    ...
    start_urls = ['https://docs.rs/tar/latest/tar/all.html']
    root_url = 'https://docs.rs'
    std_url = 'https://docs.rs/tar/latest/tar/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('tar_rust/css'):
            os.makedirs('tar_rust/css')
        html = response.text
        ...
        f = open('tar_rust/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('tar_rust/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('tar_rust/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
