...
class RustSpider(scrapy.Spider):
    ...
    start_urls = ['https://docs.rs/tantivy/0.20.2/tantivy/all.html']
    root_url = 'https://docs.rs'
    std_url = 'https://docs.rs/tantivy/0.20.2/tantivy/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('tantivy_rust/css'):
            os.makedirs('tantivy_rust/css')
        html = response.text
        ...
        f = open('tantivy_rust/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('tantivy_rust/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('tantivy_rust/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
