...
class RustSpider(scrapy.Spider):
    ...
    start_urls = ['https://docs.rs/openssl/latest/openssl/all.html']
    root_url = 'https://docs.rs'
    std_url = 'https://docs.rs/openssl/latest/openssl/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('rust_openssl/css'):
            os.makedirs('rust_openssl/css')
        html = response.text
        ...
        f = open('rust_openssl/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('rust_openssl/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('rust_openssl/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
