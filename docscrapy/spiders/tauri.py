...
class RustSpider(scrapy.Spider):
    ...
    start_urls = ['https://docs.rs/tauri/1.5.4/tauri/all.html']
    root_url = 'https://docs.rs'
    std_url = 'https://docs.rs/tauri/1.5.4/tauri/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('tauri/css'):
            os.makedirs('tauri/css')
        html = response.text
        ...
        f = open('tauri/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('tauri/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('tauri/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
