...
class RustSpider(scrapy.Spider):
    ...
    start_urls = ['https://docs.rs/sqlite/latest/sqlite/all.html']
    root_url = 'https://docs.rs'
    std_url = 'https://docs.rs/sqlite/latest/sqlite/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('sqlite/css'):
            os.makedirs('sqlite/css')
        html = response.text
        ...
        f = open('sqlite/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('sqlite/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('sqlite/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
