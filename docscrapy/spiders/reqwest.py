...
class RustSpider(scrapy.Spider):
    ...
    start_urls = ['https://docs.rs/reqwest/latest/reqwest/all.html']
    root_url = 'https://docs.rs'
    std_url = 'https://docs.rs/reqwest/latest/reqwest/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('reqwest/css'):
            os.makedirs('reqwest/css')
        html = response.text
        ...
        html_list0 = response.xpath('//ul[@class="all-items"]/li/a/@href').extract()
        ...
        f = open('reqwest/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('reqwest/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('reqwest/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
