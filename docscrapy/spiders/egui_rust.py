...
class RustSpider(scrapy.Spider):
    ...
    start_urls = ['https://docs.rs/egui/latest/egui/all.html']
    root_url = 'https://docs.rs'
    std_url = 'https://docs.rs/egui/latest/egui/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('egui_rust/css'):
            os.makedirs('egui_rust/css')
        html = response.text
        ...
        f = open('egui_rust/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('egui_rust/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('egui_rust/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
