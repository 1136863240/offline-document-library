...
class RustSpider(scrapy.Spider):
    ...
    root_url = 'https://pytorch.org'
    std_url = 'https://pytorch.org/docs/stable/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('pytorch/css'):
            os.makedirs('pytorch/css')
        html = response.text
        ...
        html_list = response.xpath('//div[@class="pytorch-menu pytorch-menu-vertical"]/ul[4]/li/a/@href').extract()
        ...
        f = open('pytorch/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('pytorch/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('pytorch/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
    ...
    def double_sub_parse(self, response):
        print('sub%d start...' % self.sub_index)
        html = response.text
        css_list = response.xpath('//link[@rel="stylesheet"]/@href').extract()
        for item in css_list:
