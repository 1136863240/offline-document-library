...
class RustSpider(scrapy.Spider):
    name = 'native_windows_gui_guide'
    allowed_domains = ['gabdube.github.io']
    start_urls = ['https://gabdube.github.io/native-windows-gui/native-windows-docs/index.html']
    root_url = 'https://gabdube.github.io'
    std_url = 'https://gabdube.github.io/native-windows-gui/native-windows-docs/'
    inner_href = []
    css_index_list = []
    sub_index = 0
    css_i = 0
    ...
    def parse(self, response):
        print('main start...')
        if not os.path.exists('native_windows_gui_guide/css'):
            os.makedirs('native_windows_gui_guide/css')
        html = response.text
        ...
        html_list = response.xpath('//ul[@class="chapter"]/li/a/@href').extract()
        for item in html_list:
            yield scrapy.Request('%s%s' % (self.std_url, item), self.sub_parse)
            html = html.replace(item, item.split('/')[-1])

        html_list = response.xpath('//ul[@class="section"]/li/a/@href').extract()
        for item in html_list:
            yield scrapy.Request('%s%s' % (self.std_url, item), self.sub_parse)
            html = html.replace(item, item.split('/')[-1])
        ...
        f = open('native_windows_gui_guide/index.html', 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
        print('main end...')
    ...
    def css_parse(self, response):
        ...
        f = open('native_windows_gui_guide/css/%s' % filename, 'w', encoding='utf-8')
    ...
    def sub_parse(self, response):
        ...
        f = open('native_windows_gui_guide/%s' % filename, 'w', encoding='utf-8')
        f.write(html)
        f.flush()
        f.close()
